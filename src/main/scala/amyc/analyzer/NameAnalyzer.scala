...
object NameAnalyzer extends Pipeline[N.Program, (S.Program, SymbolTable)] {
  def run(ctx: Context)(p: N.Program): (S.Program, SymbolTable) = {
    ...
    for {
      ...
      val retType = table
        .getType(m.name, parent)
        .getOrElse(fatal(s"Parent class $parent not found", cc))
    ...
    def transformExpr(expr: N.Expr)(implicit
        module: String,
        names: (Map[String, Identifier], Map[String, Identifier])
    ): S.Expr = {
      ...
      val res = expr match {
        case N.Variable(name) =>
          S.Variable(
            locals.getOrElse(
              name, // Local variables shadow parameters!
              params.getOrElse(name, fatal(s"Variable $name not found", expr))
            )
          )
        ...
        case N.CharLiteral(value, isEscape) =>
          S.CharLiteral(value, isEscape)
        ...
        case N.Match(scrut, cases) =>
          def transformCase(cse: N.MatchCase) = {
            ...
            S.MatchCase(
              newPat,
              transformExpr(rhs)(module, (params, locals ++ moreLocals))
                .setPos(rhs)
            ).setPos(cse)
          }
          ...
          def transformPattern(
              pat: N.Pattern
          ): (S.Pattern, List[(String, Identifier)]) = {
            val (newPat, newNames): (S.Pattern, List[(String, Identifier)]) =
              pat match {
                ...
                case N.LiteralPattern(lit) =>
                  (
                    S.LiteralPattern(
                      transformExpr(lit).asInstanceOf[S.Literal[_]]
                    ),
                    List()
                  )
                case N.CaseClassPattern(constr, args) =>
                  val (sym, sig) = table
                    .getConstructor(
                      constr.module.getOrElse(module),
                      constr.name
                    )
                  ...
                  val (newPatts, moreLocals0) =
                    (args map transformPattern).unzip
