...
trait Printer {
  ...
  implicit def printQName(name: QualifiedName)(implicit
      printUniqueIds: Boolean
  ): Document
    ...
    def rec(t: Tree, parens: Boolean = true): Document = t match {
      ...
      case ModuleDef(name, defs, optExpr) =>
        Stacked(
          ...
          Indented(
            Stacked(
              defs ++ optExpr.toList map (rec(_, false)),
              emptyLines = true
            )
          ),
          "end " <:> name,
      ...
      case CaseClassDef(name, fields, parent) =>
        ...
        "case class " <:> name <:> "(" <:> Lined(
          fields map printField,
          ", "
        ) <:> ") extends " <:> parent
      ...
      case BooleanLiteral(value) =>
        value.toString
      case CharLiteral(value, isEscape) =>
        "'" + (if isEscape then "\\" else "") + value.toString + "'"
        value.toString
      ...
      case Let(df, value, body) =>
        val main = Stacked(
          ...
          rec(
            body,
            false
          ) // For demonstration purposes, the scope or df is indented
        )
        if (parens) {
          Stacked(
            "(",
...
object NominalPrinter extends Printer {
  ...
  implicit def printName(name: Name)(implicit
      printUniqueIds: Boolean
  ): Document = Raw(name)

  implicit def printQName(
      name: QualifiedName
  )(implicit printUniqueIds: Boolean): Document = {
    Raw(name match {
...
trait SymbolicPrinter extends Printer {
  ...
  @inline implicit def printQName(
      name: QualifiedName
  )(implicit printUniqueIds: Boolean): Document = {
    printName(name)
  }
